...
public final class ApiClient {
    ...
    public ApiResponse get(String path, Map<String, String> headers) {
        HttpRequest request = requestBuilder(path, headers)
                .GET()
                .build();
        ...
        for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
                ...
                if (attempt == MAX_ATTEMPTS) {
                    throw new IllegalStateException("HTTP-запрос завершился ошибкой: " + method + " " + path, exception);
                }
            } catch (InterruptedException exception) {
                Thread.currentThread().interrupt();
                throw new IllegalStateException("HTTP-запрос был прерван: " + method + " " + path, exception);
            }
        }

        throw new IllegalStateException("HTTP-запрос завершился ошибкой после повторов: " + method + " " + path);
    }
