...
public class GetStatisticApiTest extends ApiTestBase {
    ...
    void tc62_repeatGetStatisticShouldReturnStableData() {
        ...
        ApiResponse secondResponse = statisticEndpoint.getByItemId(createdItem.id(), acceptJsonHeader());
        ...
        assertEquals(firstStatistics, secondStatistics, "[TC-62] Данные статистики должны совпадать между повторными GET");
    }
    ...
    void tc66_getStatisticWithCorrectAcceptHeader() {
        ...
        assertEquals(200, response.statusCode(), "[TC-66] Ожидался статус 200 с Accept: application/json");
    ...
    void tc75to76_getStatisticWithMissingOrInvalidAccept(HeaderScenario scenario) {
        ...
        if (response.statusCode() == 200) {
            assertStatisticContract(firstObject(response.json(), scenario.caseId() + " ответ"), scenario.caseId());
        }
    }
}
